...
{
  "version": 1,
  "updated_at": "2026-09-23T16:03:34Z",
  "metadata": {},
  "providers": {
    "openrouter": {
      "metadata": {},
      "models": [
        ...
        {"id": "deepseek/deepseek-v4.1-flash", "description": "reasoning: low → max"}
      ]
    },
    "nous": {
      "metadata": {},
...
Google's current stable Flash inventory now includes `gemini-3.8-flash`, alongside retained 3.7, 3.6, and 3.5 compatibility entries. Google identifies 3.8 as the new stable model and 3.7 as the previous-generation stable model. Google also lists stable Live API IDs `gemini-3.8-live` and `gemini-3.8-live-extended-thinking`, released 2026-09-15, and recommends them over legacy `gemini-3.1-flash-live-preview`. This verification was refreshed on 2026-09-25. Google also lists specialized stable TTS and transcription endpoints; they are outside this conversational/tool-calling picker manifest and are not added here. Google lists `gemini-3.1-flash-lite` as stable, with a provider-announced shutdown on 2026-05-07, and confirms that `gemini-3.1-flash-lite-preview` shut down on 2026-05-25; retain the stable ID and do not change runtime defaults from this catalog.
...
The 2026-09-25 current set includes Claude Fable 5.1, Opus 5.5, Sonnet 5, GPT-6 Astra/Sol/Luna, Gemini 3.8 Flash, Gemini 3.7 Flash, Gemini 3.6 Flash, Gemini 3.5 Flash and Flash-Lite, Gemini 3.1 Flash-Lite, Gemini 3.1 Pro (preview), DeepSeek V4.1 Flash and V4 Pro, and Gemma 4 open-weight checkpoints. Google’s current model page also lists specialized TTS and transcription endpoints; they remain outside this conversational/tool-calling manifest. Google’s current deprecations page lists Gemini 3.1 Flash-Lite as stable, with a May 7, 2026 shutdown date and Gemini 3.5 Flash-Lite as the recommended replacement. OpenAI documents the GPT-6 IDs and reasoning controls; OpenRouter availability is checked against its model registry, while provider access, release status, and licensing remain separate claims. Registry-only candidates, including the Qwen3.6 URL’s redirect to Qwen3.8, GLM 5.3, and Grok 4.6 entries whose primary release or provider evidence is absent, remain explicitly unverified in the shared catalog watchlist.
