...
DeepSeek's official change log documents GA `deepseek-v4-pro` and public-beta `deepseek-v4-flash`, with `low`, `high`, and `max` thinking effort. It announced that the legacy `deepseek-chat` and `deepseek-reasoner` names would be discontinued on 2026-07-24, but the current changelog does not separately confirm completion. OpenRouter exposes exact IDs for both current V4 models, so they are included in the OpenRouter manifest; they are not added to the Nous Portal block without Portal evidence.
